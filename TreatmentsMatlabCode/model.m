...
%------------------------------- Parameters ---------------------------------
tau1  = pars(45); TM   = pars(46); kt    = pars(48);
...
%------------------------ Antipyretic Temperature Interventions-------------
...
if intAntipyret
    ...
    elseif t > 7.0 && t < 10.0
        % do nothing
    elseif t > 10.0 && t < 13.0
        kttnf = 0.1*kttnf;
        kt6 = 0.1*kt6;	
    elseif t > 13.0 && t < 16.0
        % do nothing
    end
end
...
%--------------------------- Pain Perception Equations ----------------------------
%------------------------------- Parameters ---------------------------------
%PPT parameters & equation
ppM   = pars(64); kpepp = pars(65); kpp   = pars(66);

%------------------------antipyretic pain perception interventions-------------
...
%---------------------------- Cardio Model Equations -----------------------------
%------------------------------- Parameters ---------------------------------
Ra  = pars(67); Rv  = pars(68); Rs  = pars(69);
Cla = pars(70); Csa = pars(71); Clv = pars(72); Csv = pars(73);
Em  = pars(74); EM  = pars(75);
...
Vstr = -(pla/EM - plv/Em); %The k's are very close to 0
Q    = Vstr*hr/60;
